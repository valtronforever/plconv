...
@Gtk.Template.from_file(str(Path(__file__).parents[0] / 'layout_multiple.glade'))
class LayoutMultiple(Gtk.Box):
    __gtype_name__ = 'LayoutMultiple'

    supplier: Supplier
    in_path_1 = Gtk.Template.Child()
    in_path_2 = Gtk.Template.Child()
    ...
    def __init__(self, supplier: Supplier, **kwargs):
        ...
        self.in_path_1.connect('file-set', self.render_button_sensivity)
    ...
    def on_convert_button_clicked(self, button):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            in_path_list = [Path(self.in_path_1.get_filename())]
            if self.in_path_2.get_filename() is not None:
                in_path_list.append(
                    Path(self.in_path_2.get_filename())
                )
    ...
    def render_button_sensivity(self, *args, **kwargs):
        if self.in_path_1.get_filename() is not None and \
