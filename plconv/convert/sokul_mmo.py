...
def sokul_mmo(params: ConvertParams):
    ...
    df_head = df[:3]
    df_body = df[3:]
    df_body[19] = df_body[19].apply(lambda x: float(Decimal(Decimal(x) / Decimal(1.2)).quantize(Decimal('1.00'))))
    df_body[15] = df_body[15].apply(lambda x: int(float(x)))
